...
struct Node {
    long long sum = 0;     
    long long set_val = LLONG_MIN; 
    long long add_val = 0;   
};

void push(int node, int left, int right, vector<Node>& tree) {
    if (tree[node].set_val != LLONG_MIN) {
...
void add(int l, int r, long long val, int node, int left, int right, vector<Node>& tree) {
...
void set_val(int l, int r, long long val, int node, int left, int right, vector<Node>& tree) {
    push(node, left, right, tree);
    if (r < left || l > right) return;
    if (l <= left && right <= r) {
...
long long getSum(int l, int r, int node, int left, int right, vector<Node>& tree) {
    push(node, left, right, tree);
